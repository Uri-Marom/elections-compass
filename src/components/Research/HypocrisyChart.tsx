...
// 100 = fully consistent, 0 = maximum divergence
...
export function HypocrisyChart({ results, parties, questions, lang }: Props) {
  ...
  function getParty(id: string) { return parties.find(p => p.id === id) }
  function getQuestion(qid: string) { return questions.find(q => q.id === qid) }
        ...
        return (
          <div key={result.party_id} className="border border-gray-200 rounded-xl overflow-hidden bg-white">
            ...
            >
              ...
              {/* Party color dot */}
              <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: party.color }} />

              {/* Party name */}
              <span className="flex-1 text-sm font-medium text-gray-900 text-start">{name}</span>

              {/* Consistency label */}
              <span className="text-xs shrink-0" style={{ color }}>{consistencyLabel(result.score, lang)}</span>

              {/* Score number */}
              <span className="text-sm font-bold shrink-0 w-8 text-end" style={{ color }}>{score}</span>
            ...
            {isExpanded && (
              <div className="px-4 pb-4 border-t border-gray-100 bg-gray-50">
                {result.topGaps.length === 0 ? (
                  <p className="text-xs text-gray-400 mt-3">{t('no_gaps_found')}</p>
                ) : (
                  <>
